...
def generate_briefing(store: GraphStore, repo_root: str) -> str:
    ...
    # Storage / privacy note — brain.db is extracted source metadata.
    gitignore_hint = _brain_gitignore_status(repo_root)
    lines.append("## Storage")
    lines.append(
        "  `.nervx/brain.db` contains extracted signatures, docstrings, and "
        "call-graph metadata derived from this repo's source code. It is "
        "regenerated by `nervx build` / `nervx update` and should usually be "
        "treated as a local cache."
    )
    if gitignore_hint:
        lines.append(f"  {gitignore_hint}")
    lines.append("")

    return "\n".join(lines)


def _brain_gitignore_status(repo_root: str) -> str:
    """Return a one-line hint if `.nervx/` isn't covered by .gitignore."""
    ...
    covered = any(p in patterns for p in (".nervx", ".nervx/", ".nervx/*", "*.db"))
    if covered:
        return ""
    return (
        "⚠ `.nervx/` is not listed in `.gitignore`. Add it unless you "
        "intend to commit source metadata."
    )
...
`server/main.py::handle_request`. Fuzzy matching is built in:
- Short names like `handle_request` resolve automatically when unambiguous.
- `Class.method` shorthand (e.g. `RadixCache.insert`) beats plain-name ties,
  so you rarely need the full `file::` prefix.
- Ambiguous queries return a "did you mean?" list.
